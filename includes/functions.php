...
function serverResponse($server, $url = '', $vars = array())
{
...
function checkCsfAlive($server)
{
	...
	{
		$response = serverResponse($server, $file);
...
function csfPagination($start, $per_page, $num_items, $base_url, $add_prevnext_text = true)
{
	global $LANG;
	...
	if($current_page == 1)
	{
		$display_output .= "<li><a class='current_page' href='{$base_url}{$url_delim}start=0'><span><span>1</span></span></a></li>";
	}
	else
	{
		$display_output .= "<li><a href='{$base_url}{$url_delim}start=0'>1</a></li>";
	}

	if ($total_pages > 5)
	{
		$start_cnt = min(max(1, $current_page - 4), $total_pages - 5);
		$end_cnt = max(min($total_pages, $current_page + 4), 6);

		if($start_cnt > 1)
		{
			$display_output .= '<li>[...]</li>';
		}
		...
		for ($i = $start_cnt + 1; $i < $end_cnt; $i++)
		{
			if($i == $current_page)
			{
				$display_output .= "<li><a class='current_page' href='{$base_url}{$url_delim}start=" . (($i - 1) * $per_page) . "'><span><span>{$i}</span></span></a></li>";
			}
			else
			{
				$display_output .= "<li><a href='{$base_url}{$url_delim}start=" . (($i - 1) * $per_page) . "'>{$i}</a></li>";
			}
		}
	...
	{
		$display_output .= "<li><a class='current_page' href='{$base_url}{$url_delim}start=" . (($total_pages - 1) * $per_page) . "'><span><span>{$total_pages}</span></span></a></li>";
	}
	else
	{
		$display_output .= "<li><a href='{$base_url}{$url_delim}start=" . (($total_pages - 1) * $per_page) . "'>{$total_pages}</a></li>";
	}

	$output = "<ul>" . $display_output . "</ul>";

	if ($add_prevnext_text)
	{
		$output = '';
		$prev_disabled = ($current_page == 1) ? true : false;
		$next_disabled = ($current_page < $total_pages) ? false : true;

		$output .= "<ul>";
		if(!$prev_disabled) $output .= "<li><a href='{$base_url}{$url_delim}start=" . (($current_page - 2) * $per_page) . "' class='pag_nav'><span><span>&laquo; {$LANG['prevpage']}</span></span></a></li>";
		$output .= $display_output;
		if(!$next_disabled) $output .= "<li><a href='{$base_url}{$url_delim}start=" . ($current_page * $per_page) . "' class='pag_nav'><span><span>{$LANG['nextpage']} &raquo;</span></span></a></li>";
		$output .= "</ul>";
	}
...
function csfOnpage($num_items, $per_page, $start)
{
	global $LANG;
	...
	return sprintf($LANG['onpage'], $num_items,$on_page, max(ceil($num_items / $per_page), 1));
}

function sortby(&$data, $keyname, $order = 'ASC')
{
...
function sendCSFmail($messagename, $email, $fullname, $email_fields = array())
{
...
function csfValidateEmail($email)
{
